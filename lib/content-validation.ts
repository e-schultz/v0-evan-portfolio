...
export const validateContentBlock: Validator<ContentBlock> = (block) => {
  ...
  if (
    ...
    errors.push({
      ...
      message: `${block.type} block must have content`,
    })
  }
  ...
  if (project.screenshots) {
    if (!Array.isArray(project.screenshots)) {
      ...
      project.screenshots.forEach((screenshot, index) => {
        if (!screenshot.image) {
          errors.push({
            field: `screenshots[${index}].image`,
            message: 'Screenshot must have an image URL',
          })
        }
        if (!screenshot.alt) {
          errors.push({
            field: `screenshots[${index}].alt`,
            message: 'Screenshot must have alt text',
          })
        }
      })
    }
