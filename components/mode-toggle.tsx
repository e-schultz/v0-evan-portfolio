    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-[8rem]">
        {availableThemes.map((t) => (
          <DropdownMenuItem key={t} onClick={() => setTheme(t)} className="flex items-center gap-2 capitalize">
            {getThemeIcon(t)}
            <span>{t}</span>
            {theme === t && <span className="absolute right-2 flex h-2 w-2 rounded-full bg-primary"></span>}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
